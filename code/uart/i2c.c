...
union TWI_statusReg TWI_statusReg = {0}; // TWI_statusReg is defined in i2c.h
...
{
	msg[0] &= ~(TWI_READ);// Be sure read/write bit is clear (write) regardless.
...
ISR(TWI_vect)
{
	...
	switch (TWSR & 0xf8) // Mask off prescaler bits
	{
		...
		case
 TWI_MTX_DATA_ACK: // Data byte has been tramsmitted and ACK received
